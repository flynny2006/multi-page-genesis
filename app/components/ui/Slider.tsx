...
export type SliderOptions<T> = {
  left: { value: T; text: string; disabled?: boolean };
  middle?: { value: T; text: string; disabled?: boolean };
  right: { value: T; text: string; disabled?: boolean };
};
  ...
  return (
    <div className="flex items-center flex-wrap shrink-0 gap-1 bg-bolt-elements-background-depth-1 overflow-hidden rounded-full p-1">
      <SliderButton 
        ...
        disabled={options.left.disabled}
      >
        {options.left.text}
      </SliderButton>
      ...
      {options.middle && (
        <SliderButton 
          ...
          disabled={options.middle.disabled}
        >
          {options.middle.text}
        </SliderButton>
      )}
      ...
      <SliderButton
        ...
        disabled={options.right.disabled}
      >
        {options.right.text}
      </SliderButton>
    </div>
...
const SliderButton = memo(({ selected, children, setSelected, disabled }: SliderButtonProps) => {
  return (
    <button
      onClick={setSelected}
      disabled={disabled}

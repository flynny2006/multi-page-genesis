...
import CreditsTab from '../tabs/credits/CreditsTab';
...
export const ControlPanel = ({ open, onClose }: ControlPanelProps) => {
  ...
  const filteredTabs = useMemo(() => {
    ...
    return (visibleTabs as TabWithDevType[]).filter((tab) => {
      const title = TAB_LABELS[tab.id].toLowerCase();
      const description = TAB_DESCRIPTIONS[tab.id].toLowerCase();
  ...
  useEffect(() => {
    if (!open) {
      ...
      setActiveTab(null);
      // Reset tab configuration to ensure credits tab is visible
      resetTabConfiguration();
    }
  }, [open]);
          ...
          >
            ...
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-red-100 dark:border-red-800/50 bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm">
                        ...
                        <div className="relative">
                          ...
                          {searchQuery && (
                            <button
                              ...
                              className="absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                            >
                              <div className="i-ph:x w-3 h-3 text-gray-400 dark:text-gray-500" />
                            </button>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="flex items-center gap-6">
                    {/* Avatar and Dropdown */}
                    <div className="border-l border-red-100 dark:border-red-800/50 pl-6">
                ...
                <div
                  className={classNames(
                    ...
                    'scrollbar-thumb-red-200 hover:scrollbar-thumb-red-300',
                    'dark:scrollbar-thumb-red-800/50 dark:hover:scrollbar-thumb-red-700/50',
                    'will-change-scroll',
                    'touch-auto',
                  )}
                >
                    ...
                    {showTabManagement ? (
                      ...
                      >
                        <AnimatePresence mode="popLayout">
                          {filteredTabs.map((tab: TabWithDevType) => (
                ...
                {/* Developer Mode Toggle - Now at bottom */}
                <div className="border-t border-red-100 dark:border-red-800/50 bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm p-4">
                  <div className="flex items-center justify-between max-w-md mx-auto">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-red-100 dark:bg-red-900/20 flex items-center justify-center">
                        <div className="i-ph:code w-4 h-4 text-red-500" />
                      </div>
                      <div>
                        <div className="text-sm font-medium text-gray-900 dark:text-white">Developer Mode</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">Access advanced features and settings</div>
                      </div>
                    </div>
                    <AnimatedSwitch
                      id="developer-mode"
                      ...
                      label={developerMode ? 'Enabled' : 'Disabled'}
                    />
                  </div>
                </div>
              </div>

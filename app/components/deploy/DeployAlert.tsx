...
export default function DeployChatAlert({ alert, clearAlert, postMessage }: DeployAlertProps) {
  const { type, title: originalTitle, description: originalDescription, content, url, stage, buildStatus, deployStatus } = alert;
  ...
  if (type === 'success') {
    ...
    description = 'The last build was successful. Read the preview to see it in real-time.';
    ...
    iconClass = 'i-ph:info-duotone text-bolt-elements-loader-progress';
  }

  // Determine if we should show the deployment progress
  const showProgress = type !== 'success' && stage && (buildStatus || deployStatus);
